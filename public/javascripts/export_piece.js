createCanvas_both = function () {
  const pArtist = document.getElementById("artist-span").innerHTML.trim();
  const pTitle = document.getElementById("title-span").innerHTML.trim();
  const pCatNum = document.getElementById("catalogue-span").innerHTML.trim();
  const pMedium = document.getElementById("medium-span").innerHTML.trim();
  const pYear = document.getElementById("year-span").innerHTML.trim();
  const pSize = document.getElementById("size-span").innerHTML.trim();
  let pOwner;
  let pOContact;
  ...
  const pPrice = document.getElementById("price-span");

  pTitle += ", ";
  const lineMedium = pMedium + " " + pSize;

  const createCanvas_mus = function () {
    ls = 55; // line spacing in px
    text_X = 50;
    text_Y = 100;

    const canvas = document.getElementById("canvas_mus");
  ...
  let createCanvas_cat = async function () {
    ...
    const pieceInfo_X = 450;
    const pieceInfo_Y = 264;

    const ownerInfo_X = 450;
    const ownerInfo_Y = 516;

    const pieceImg_X = 75;
    const pieceImg_Y = 192;

    const catNum_X = ownerInfo_X + 200;
    const catNum_Y = 80;
  ...
  let createCanvasLink = function (canvasId, linkId) {
    $(linkId).click(function () {
      $(this)
        .parent()
        .attr("href", document.querySelector(canvasId).toDataURL());
      $(this)
        .parent()
        .attr(
          "download",
          `${canvasId.substring(1)}_label_for_${pTitle}_by_${pArtist}.png`
        );
    });
  };
